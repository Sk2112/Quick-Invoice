    ...
    <div className="template1 border rounded mx-auto my-4 px-sm-4 py-3 w-100">
      ...
      <div className="row mb-4">
        ...
        <div className="col-md-6 text-start text-md-end">
          <h1 className="mb-2 invoice-title">{data.title}</h1>
      ...
      <div className="mb-4">
        <div className="table-responsive">
          <table className="table table-bordered">
            <thead>
              <tr>
                <th className="p-2 table-header">Serial No.</th>
                <th className="p-2 table-header">Item</th>
                <th className="p-2 table-header">Description</th>
            ...
            <tbody>
              {(data.items || []).map((item, index) => (
                <tr key={index}>
                  ...
                  <td className="p-2 text-end">
                    Rs {(Number(item.qty || 0) * Number(item.amount || 0)).toFixed(2)}
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="p-3 w-100 totals-box" style={{ maxWidth: '300px' }}>
            ...
            {Number(data.tax) > 0 && (
              <div className="d-flex justify-content-between mb-2">
                ...
                <span>Rs {Number(data.taxAmount || 0).toFixed(2)}</span>
              </div>
            )}
            <div className="d-flex justify-content-between fw-bold total-highlight">
              <span>Total:</span>
              <span>Rs {Number(data.total || 0).toFixed(2)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {(data.accountName || data.accountNumber || data.accountIfscCode) && (
        <div className="mt-4">
          ...
          {data.accountIfscCode && (
            <p className="mb-0">
              <strong>IFSC Code / Branch Code:</strong> {data.accountIfscCode}
            </p>
          )}
        </div>
      )}
